...
export function createAgentScopedService(
  ...
  return {
    async getAgents(accountId: string): Promise<Agent[]> {
    ...
    async getAgent(agentId: string, accountId: string): Promise<Agent | null> {
    ...
    async createAgent(
      ...
      const apiKeyResult = await apiKeyService
        .scoped(clients)
        .createApiKey(agentUserId, { service: 'core-api' });
    ...
    async deleteAgent(
      ...
      await apiKeyService.scoped(clients).revokeApiKey(agentId, 'core-api');
