...
export const userMutations = {
  ...
  createAgentProfile: (
    ...
    { previousAgentProfiles?: ProfileUI[] }
  > => ({
    mutationFn: async (payload) => {
      const { data } = await apiClient.post<ProfileAPI>(
        `/api/profiles/agents`,
        {
          displayName: payload.displayName,
          avatarUrl: payload.avatarUrl,
    ...
    onMutate: async (payload) => {
      // Cancel outgoing refetches
      await queryClient.cancelQueries({ queryKey: userKeys.agentProfiles() });

      // Snapshot previous value
      const previousAgentProfiles = queryClient.getQueryData<ProfileUI[]>(
        userKeys.agentProfiles()
      );

      // Optimistically add agent profile
      const optimisticProfile: ProfileUI = {
        userId: 'temp-' + crypto.randomUUID(),
        profileId: 'temp-' + crypto.randomUUID(),
        type: 'agent',
      ...
      const updatedAgentProfiles = [
        ...(previousAgentProfiles || []),
        optimisticProfile,
      ];

      queryClient.setQueryData<ProfileUI[]>(
        userKeys.agentProfiles(),
        updatedAgentProfiles
      );

      return { previousAgentProfiles };
    },

    onError: (_err, _payload, context) => {
      // Rollback on error
      if (context?.previousAgentProfiles) {
        queryClient.setQueryData(
          userKeys.agentProfiles(),
          context.previousAgentProfiles
        );
      } else {
        queryClient.removeQueries({ queryKey: userKeys.agentProfiles() });
      }
    },

    onSuccess: () => {
      // Invalidate to refetch with server data
      queryClient.invalidateQueries({ queryKey: userKeys.agentProfiles() });
    },
  }),
