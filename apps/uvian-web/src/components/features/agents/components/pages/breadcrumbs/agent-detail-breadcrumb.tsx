...
import { Home, Bot, ChevronRight, Users } from 'lucide-react';
import {
  useIsMobile,
  Sheet,
  SheetContent,
...
interface AgentDetailPageBreadcrumbProps {
  accountId: string;
  agentId: string;
}

export function AgentDetailPageBreadcrumb({
  accountId,
  agentId,
}: AgentDetailPageBreadcrumbProps) {
  const isMobile = useIsMobile();
  const [open, setOpen] = useState(false);

  const currentPageTitle = 'Agent';
    ...
    return (
      <Breadcrumb>
        <BreadcrumbList>
          ...
          <BreadcrumbItem>
            <BreadcrumbLink asChild>
              <Link href={`/accounts/${accountId}/agents`}>Agents</Link>
            </BreadcrumbLink>
          </BreadcrumbItem>
          <BreadcrumbSeparator />
      ...
      <Sheet open={open} onOpenChange={setOpen}>
        <SheetContent side="bottom" className="h-[60vh] rounded-t-xl">
          ...
          <nav className="flex flex-col gap-1 mt-4">
            ...
            <Link
              href={`/accounts/${accountId}/agents`}
              onClick={() => setOpen(false)}
              className="flex items-center gap-3 p-3 rounded-lg hover:bg-accent transition-colors"
            >
