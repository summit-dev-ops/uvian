...
import { Home, Folder, Pencil, ChevronRight } from 'lucide-react';
import {
  useIsMobile,
  Sheet,
  SheetContent,
...
export function SpaceEditPageBreadcrumb({ spaceId }: { spaceId: string }) {
  ...
  const currentPageTitle = 'Edit Space';
  const parentTitle = space?.name || 'Space';
      ...
      <Sheet open={open} onOpenChange={setOpen}>
        <SheetContent side="bottom" className="h-[60vh] rounded-t-xl">
          ...
          <nav className="flex flex-col gap-1 mt-4">
            ...
            >
              <Folder className="h-5 w-5 text-muted-foreground" />
              <span>{isLoading ? 'Loading...' : parentTitle}</span>
            </Link>
