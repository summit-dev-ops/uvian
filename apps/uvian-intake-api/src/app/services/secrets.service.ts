...
export class SecretsService {
  ...
  async get(userClient: SupabaseClient, accountId: string, secretId: string) {
    const { data, error } = await userClient
      .schema('public')
      .from('secrets')
      .select('*')
  ...
  async getByIdWithDecryptedValue(secretId: string): Promise<{
    ...
    console.log(
      '[secretsService.getByIdWithDecryptedValue] Fetching secret:',
      secretId
    );
    const { data, error } = await adminSupabase
      .schema('public')
  ...
  getDecryptedValue(encryptedValue: string): string {
    return decrypt(encryptedValue, ENCRYPTION_SECRET);
  }
