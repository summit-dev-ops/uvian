...
```
uvian-hub-api (BullMQ: uvian-events) → uvian-hub-event-worker → EventRouter
                                                      │
                                    ┌─────────────────┴─────────────────┐
                                    ▼                                   ▼
...
`uvian-hub-api` emits events to BullMQ `uvian-events` queue → `uvian-hub-event-worker` routes to subscribers → `uvian-automation-api` receives internal events → creates jobs → `uvian-automation-worker` executes.
